...
def test_chamfer(distChamfer, dim):
    ...
    points1 = torch.rand(1, k, dim).cuda()
    
    L = torch.Tensor([1,0,1,0,0,1]).repeat(k, 1).reshape((1,k,6)).cuda()
    points2 = torch.rand(1, k, dim, requires_grad=True).cuda()
    start = time.time()
    dist1, dist2, idx1, idx2= distChamfer(points1, points2, L)
    #print(idx1, idx2)
...
def timings(distChamfer, dim):
    p1 = torch.rand(1, 500, dim).cuda()
    p2 = torch.rand(1, 500, dim).cuda()
